...
export default function Index() {
    ...
    const fetchDataOrder = async () => {
      try {
        if (session?.user?.id_user) {
          const response = await axios.get(`${testURL}/api/daftardikirimcust?id=${session.user.id_user}`);
  ...
  const handleSelesai = async (idBeli) => {
    try {
      const response = await axios.post(`${testURL}/api/selesai-status`, {
        id_beli: idBeli,
      });
  ...
  const handleView = (beliId)=>{
    router.push(`/order/dikirim/${beliId}`);
  };
   ...
   <nav className="bg-white text-gray-800 py-8 shadow-lg">
      <div className="container mx-auto flex flex-col items-center lg:flex-row lg:justify-between px-4">
        ...
        >
          <li>
            <Link href="/order">
              <p className="hover:text-blue-900">Semua Pesanan</p>
            </Link>
          </li>
          <li>
            <Link href="/order/masuk">
              <p className="text-blue-900">Masuk</p>
            </Link>
          </li>
          <li>
          ...
          <li>
            <Link href="/order/dikirim">
              <p className="text-blue-900">Dikirim</p>
            </Link>
          </li>
          <li>
            ...
            <div className="flex flex-wrap items-center gap-y-4 p-6">
              ...
              <div className="w-full grid sm:grid-cols-2 lg:flex lg:w-64 lg:items-center lg:justify-end gap-4">
                  ...
                  <>
                    <button
                      type="button"
                      onClick={() => handleSelesai(order.id_beli)}
                      className="w-full rounded-lg border border-black px-3 py-2 bg-slate-800 text-center text-sm font-medium text-green-500 hover:bg-green-700 hover:text-white focus:outline-none focus:ring-4 focus:ring-red-300 lg:w-auto"
                    >
                      Selesaikan Pesanan
                    </button>
                    <button
