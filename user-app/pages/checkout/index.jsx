...
export default function  Index() {
  ...
  const [codeSubdistric, setCodeSubdistric] = useState(1);
  ...
  const { data: session, status } = useSession();
   ...
   useEffect(() => {
      ...
      const fetchCities = async () => {
        try {
          const response = await axios.get(
            `${testURL}/api/ongkir/kota?province_id=${dataProv}`,
          );
    ...
    useEffect(() => {
        const fetchSubdistricts = async () => {
          if (!dataKot) return;
          try {
            const response = await axios.get(`${testURL}/api/ongkir/subdistrict?city_id=${dataKot}`);
    ...
    useEffect(() => {
      const checkCostService = async () => {
        const tempCodeCities = parseInt(dataKot);
   ...
   useEffect(() => {
    ...
    const combineAddress = () => {
      return `${dataAlamat_lengkap}, Kecamatan ${dataKecamatan}, Kelurahan ${dataKelurahan}, Provinsi ${dataProvinsi},Kota ${dataKota} ,Kode Pos ${dataKodePos}`;
    };
