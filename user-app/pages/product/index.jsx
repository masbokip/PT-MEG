...
export default function ProductList() {
  ...
  try {
    const response = await axios.get(`${testURL}/api/products`, {
      params: { search: query },
    });
  ...
  if (loading) {
    return <p className="text-center text-gray-800 bg-white">Loading...</p>;
  }
      ...
      <nav className="bg-white text-gray-800 py-8 drop-shadow-lg">
        <div className="container mx-auto flex justify-between items-center px-4">
          ...
          <ul className={`lg:flex space-x-4 ${isMenuOpen ? 'block' : 'hidden'}`}>
            <li>
              <Link href="/product">
                <p className="text-blue-900">Urutkan</p>
              </Link>
            </li>
            <li>
              <Link href="/product/filter/termahal">
