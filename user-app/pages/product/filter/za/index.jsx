...
import { FaCartShopping, FaHeart, FaShieldHalved, FaLayerGroup, FaBoxOpen, FaSearchengin, FaArrowDown } from 'react-icons/fa6';
...
export default function ProductList() {
  ...
  const fetchProductData = async (query = "") => {
    try {
      const response = await axios.get(`${testURL}/api/productsZA`, {
        params: { search: query },
      });
    ...
    <>
      <nav className="bg-white text-gray-800 py-8 drop-shadow-lg">
        <div className="container mx-auto flex justify-between items-center px-4">
          ...
          <button
            className="block lg:hidden p-2 text-gray-700 hover:bg-gray-100 rounded"
          ...
          <ul className={`lg:flex space-x-4 ${isMenuOpen ? 'block' : 'hidden'}`}>
            <li>
              <Link href="/product">
                <p className="hover:text-blue-900">Semua Produk</p>
              </Link>
            </li>
            <li>
              <Link href="/product/filter/termurah">
                <p className="hover:text-blue-900">Harga</p>
              </Link>
            </li>
            <li>
              <Link href="/product/filter/terbaru">
                <p className="hover:text-blue-900">Rilis</p>
              </Link>
            </li>
            <li>
              <Link href="/product/filter/az">
              <p className="flex items-center text-blue-600">
              Nama (Z~a) <FaArrowDown className="ml-1" />
             </p>
              </Link>
            </li>
          </ul>
                          ...
                          {currentItems.map(product => (
                            <div key={product.id_produk} className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                              ...
                              <div className="pt-6">
                                ...
                                <a href="#" className="text-lg font-semibold leading-tight text-gray-900 hover:underline " onClick={() => handleDetail(product.id_produk)}>
                                  {product.nama_produk}
                                </a>
