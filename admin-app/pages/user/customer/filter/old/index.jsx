...
export default function Page() {
    ...
    const fetchData = async (query = "") => {
      try {
        const response = await axios.get(`${testURL}/api/usersCSTold`, {
          params: { search: query },
        });
     ...
     <nav className="rounded-lg border border-gray-800 bg-gray-900 p-4 shadow-lg mt-5">
      ...
      <div className="flex items-center space-x-4">
        ...
        <ul className="flex space-x-4 mx-6">
          ...
          <li>
            <Link href="/user/pegawai">
              <p className="text-green-400">Customer</p>
            </Link>
          </li>
          <li>
            <Link href="/user/customer/filter/az">
            <p className="hover:text-green-400">Semua User</p>
            </Link>
          </li>
          <li>
            <Link href="/user/customer/filter/new">
            <p className="flex items-center text-green-400">
              Terlama <FiArrowDown className="ml-1" />
             </p>
            </Link>
          </li>
        </ul>
