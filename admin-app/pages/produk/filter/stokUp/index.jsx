...
export default function Index() {
  ...
  const fetchProductData = async (query = "") => {
    try {
      const response = await axios.get(`${testURL}/api/productsPLUS`, {
        params: { search: query },
      });
  ...
  <nav className="bg-gray-900 text-white py-4 shadow-lg">
    <div className="container mx-auto flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
      ...
      <ul className="flex flex-wrap justify-center sm:justify-end space-x-4">
        ...
        <li>
          <Link href="/produk/filter/termahal">
            <p className="hover:text-green-400">Harga</p>
          </Link>
        </li>
        <li>
          <Link href="/produk/filter/stokDown">
            <p className="text-green-400">Terbanyak</p>
          </Link>
        </li>
        <li>
