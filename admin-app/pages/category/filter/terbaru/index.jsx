...
export default function CategoryPage() {
  ...
  const fetchData = async (query = "") => {
    try {
      const response = await axios.get(`${testURL}/api/categoriesNEW`, {
        params: { search: query }, 
      });
  ...
  useEffect(() => {
    fetchData(); // Fetch data initially without search
  }, []);
      ...
      <nav className="rounded-lg border border-gray-800 bg-gray-900 p-4 shadow-lg mt-5">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <ul className="flex space-x-4 mx-6">
            ...
            <li>
              <Link href="/category/filter/terlama">
                <p className="flex items-center text-green-400">
                  Terbaru <FiArrowDown className="ml-1" />
                </p>
              </Link>
            </li>
          </ul>
