...
export default function Tol() {
  ...
  const [totalPesananCK,setTotalPesananCK]=useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(`${testURL}/api/total-beli`, {
        params: { startDate, endDate },
      });
      setTotalBeli(response.data.total_beli || 0); 
    } catch (err) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${testURL}/api/daftarreport`);
  ...
  //             `${testURL}/api/daftar-beli?startDate=${startDate}&endDate=${endDate}`
  //           );
  //           setBeliData(response.data.data);
  //         } catch (error) {
  //           console.error("Failed to fetch data:", error);
  //         }
  //         setLoading(false);
...
<section className="rounded-lg border border-gray-800 bg-gray-900 p-4 shadow-lg mt-5">
      <div className="py-8 px-4 mx-auto w-full lg:py-2">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="inline-flex items-center px-5 py-2.5 mt-4 text-sm font-medium text-center text-white bg-blue-600 rounded-lg focus:ring-4 focus:ring-primary-200 hover:bg-primary-800"
          >
            Cek Pendapatan
          </button>
        </form>
...
<div className="rounded-lg border border-gray-800 bg-gray-900 p-6 shadow-lg mt-5">
        <h4 className="mb-6 text-lg font-semibold text-white">Top Daftar Pembelian yang Berhasil</h4>

        {loading ? (
          <p className="text-center text-gray-400">Loading data...</p>
        ) : (
          <>
            <div className="grid grid-cols-3 bg-gray-800 p-4 sm:grid-cols-5">
            ...
            {currentItems.map((beli, index) => (
              <div
                key={beli.id_beli}
                className="grid grid-cols-3 items-center border-b border-gray-800 p-2 sm:grid-cols-5"
              >
                ...
                {beli.status_pengiriman === 1 ? (
                    <span class="inline-flex items-center bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">
                      <span class="w-2 h-2 me-1 bg-gray-500 rounded-full"></span>
                      Belum Bayar
                    </span>
                  ) : beli.status_pengiriman === 2 ? (
                    <span className="inline-flex items-center bg-yellow-100 text-black text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">
                      <span className="w-2 h-2 me-1 bg-yellow-500 rounded-full"></span>
                      Sudah Bayar
                    </span>
                  ) : 
                  beli.status_pengiriman === 3 ? (
                    <span class="inline-flex items-center bg-blue-100 text-black text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-green-900 dark:text-green-300">
                  ...
                  (
                    <span className="inline-flex items-center bg-red-100 text-black text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-red-900 dark:text-red-300">
