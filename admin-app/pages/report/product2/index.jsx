...
export default function Tol() {
  ...
  useEffect(() => {
    const fetchProdukTerjual = async () => {
      try {
        const response = await fetch(`${testURL}/api/produk-favorit-bijian`);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
          <div className="text-xl font-semibold">{totalTerjual !== null ? totalTerjual : "Loading..."}</div>
          <div className="text-gray-400">Jumlah Produk Terjual</div>
        </div>
      </div>
  ...
  <div className="container mx-auto flex justify-center items-center">
    <ul className="flex items-center space-x-10">
      <li>
        <Link href="/report/product">
          <p className="hover:text-green-400 text-center">Produk Terlaris</p>
        </Link>
      </li>
      <li>
        <Link href="/report/product2">
          <p className="text-green-400 text-center">Produk Terfavorit</p>
        </Link>
      </li>
    </ul>
  </div>
      ...
      <div className="rounded-lg border border-gray-800 bg-gray-900 p-6 shadow-lg mt-5">
        ...
        <div className="flex flex-col">
          ...
          <div className="grid grid-cols-6 bg-gray-800 p-4">
            ...
            <div className="text-center text-sm font-semibold text-gray-400">Harga</div>
            <div className="text-center text-sm font-semibold text-gray-400">ID Produk</div>
            <div className="text-center text-sm font-semibold text-gray-400">Jumlah Favorit</div>
          </div>
          {/* Data Rows */}
          {currentItems.map((product, index) => (
            <div
              key={index}
              className="grid grid-cols-6 items-center border-b border-gray-800 p-2"
            >
