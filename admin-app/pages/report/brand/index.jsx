...
export default function Tol() {
  ...
  useEffect(() => {
    const fetchTotalCategory = async () => {
      try {
        const response = await fetch(`${testURL}/api/total-brand`);
  ...
  const fetchData = async (query = "") => {
      try {
        const response = await axios.get(`${testURL}/api/brandsreport`, {
          params: { search: query },
        });
  ...
  return (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                ...
                <div className="text-gray-400">Total Brand</div>
            </div>
    </div>
    ...
    <ul className="flex items-center space-x-10">
      ...
      <li>
        <Link href="/report/brand">
          <p className="text-green-400 text-center">Brand</p>
        </Link>
      </li>
    </ul>
  </div>
...
<div className="rounded-lg border border-gray-800 bg-gray-900 p-6 shadow-lg mt-5">
        <h4 className="mb-6 text-lg font-semibold text-white">Daftar Brand</h4>
          ...
          {currentItems.map((category, index) => (
            ...
            >
              ...
              <div className="flex items-center justify-left">
                <img
                  src={`${testURL}/assets/images/brand/${category.foto}`}
                  alt={`${category.nama_kategori} logo`}
                  className="w-20 h-auto object-contain"
                />
              </div>
              <div className="text-center text-sm font-medium text-gray-300">{category.nama_brand}</div>
